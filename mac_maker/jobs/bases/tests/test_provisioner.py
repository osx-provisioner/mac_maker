...
import pytest
from mac_maker.__helpers__.parametrize import named_ids
...
class TestJobsBase:
  ...
  @pytest.mark.parametrize(
      ...
      ids=named_ids("args:{0}"),
  )
  @pytest.mark.parametrize(
      "validity",
      ("True", "False"),
      ids=named_ids("validity:{0}", lambda arg: str(arg)[0]),
  )
  ...
  def test_precheck__valid_env__with_notes__calls_echo(
    ...
    assert mocked_click_echo.mock_calls == [
        mock.call(mocked_precheck_data['notes']),
  ...
  def test_provision__creates_inventory_file(
      self, concrete_provisioning_job: ProvisionerJobBase,
