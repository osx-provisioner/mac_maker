...
class TestSpecExtractPreCheckFromJobSpec(TestCase):
  """Test the JobSpecExtractor class extract_precheck_from_job_spec method."""

  def setUp(self) -> None:
    self.spec = precheck.PrecheckExtractor()
    self.mock_spec_root = "/root/dir1"
    self.mock_spec_file_content = {
        "workspace_root_path": self.mock_spec_root
    }
    self.mock_spec_file_data = spec.TypeSpecFileData(
  ...
  def test_get_precheck_data_result(self, m_read: mock.Mock) -> None:
    ...
    self.assertEqual(
        result,
        precheck.TypePrecheckFileData(notes=self.mock_notes, env=self.mock_env)
    )
