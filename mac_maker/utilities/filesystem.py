...
class FileSystem:
  ...
  def get_profile_data_path(self) -> Path:
    """Return the current profile's root folder location."""
    return self.work_space_root / config.PROFILE_FOLDER_PATH

  @convertable_path
  def get_roles_path(self) -> Path:
    """Return the current Ansible roles folder location(s)."""
    return self.get_profile_data_path() / "roles"

  @convertable_path
  def get_collections_path(self) -> Path:
    """Return the current Ansible collections folder location(s)."""
